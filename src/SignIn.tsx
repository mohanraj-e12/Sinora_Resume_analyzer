...
export default function SignIn({ initialEmail, showSuccessMessage, successMessage }: SignInProps = {}) {
  ...
  useEffect(() => {
    // Check props or query parameter for pre-filling email and displaying success message
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (supabaseError && !supabaseError.message.includes('FetchError')) {
      ...
      if (signedInUser) {
        await fetch('/api/auth/supabase-sync', {
          ...
          body: JSON.stringify({
            ...
            name: signedInUser.user_metadata?.full_name || signedInUser.user_metadata?.username || signedInUser.email?.split('@')[0],
            supabase_id: signedInUser.id
          })
        });
      } else {
